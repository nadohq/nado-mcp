...
Write tools: place_order, place_trigger_order, place_twap_order, cancel_and_place, cancel_orders, cancel_trigger_orders, close_position, deposit_collateral, withdraw_collateral, transfer_quote, mint_nlp, burn_nlp, link_signer.
...
- Margin mode matching: When placing trigger orders (TP/SL) or any order targeting an existing position, the marginMode and leverage MUST match the position. For isolated positions always set marginMode="isolated" with the same leverage — mismatched margin mode causes silent rejection.
